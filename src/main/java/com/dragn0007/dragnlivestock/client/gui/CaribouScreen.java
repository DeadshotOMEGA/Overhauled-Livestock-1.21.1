...
public class CaribouScreen extends AbstractContainerScreen<CaribouMenu> {
    ...
    public void renderBg(GuiGraphics graphics, float f, int i, int j) {
        ...
        RenderSystem.setShaderTexture(0, CARIBOU_INVENTORY_LOCATION);
        int x = (this.width - this.imageWidth) / 2;
        int y = (this.height - this.imageHeight) / 2;
        graphics.blit(CARIBOU_INVENTORY_LOCATION, x, y, 0, 0, this.imageWidth, this.imageHeight);


        if (this.caribou.isSaddleable()) {
            graphics.blit(CARIBOU_INVENTORY_LOCATION, x + 7, y + 17, 18, this.imageHeight + 54, 18, 18);
        }
        ...
        if (this.caribou.isFemale()) {
            if (this.caribou.isSnipped()) {
                graphics.blit(CARIBOU_INVENTORY_LOCATION, x + 161, y + 9, 107, this.imageHeight + 54, 8, 8);
            } else {
        ...
        if (this.caribou.isMale()) {
            if (this.caribou.isSnipped()) {
                graphics.blit(CARIBOU_INVENTORY_LOCATION, x + 161, y + 9, 115, this.imageHeight + 54, 8, 8);
            } else {
                graphics.blit(CARIBOU_INVENTORY_LOCATION, x + 161, y + 9, 98, this.imageHeight + 54, 8, 8);
            }
        }
    ...
    private void renderSpeedLabel(GuiGraphics graphics) {
        ...
        double speed = caribou.getAttributeBaseValue(Attributes.MOVEMENT_SPEED) * 42.16;
    ...
    private void renderJumpStrengthLabel(GuiGraphics graphics) {
        ...
        caribou.getAttributes().hasAttribute(Attributes.JUMP_STRENGTH);
        double jumpStrength = caribou.getAttributeBaseValue(Attributes.JUMP_STRENGTH);
    ...
    private void renderHealthLabel(GuiGraphics graphics) {
        String text = String.valueOf(this.caribou.getMaxHealth());
