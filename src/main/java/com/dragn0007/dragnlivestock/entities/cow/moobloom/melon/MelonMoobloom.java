...
public class MelonMoobloom extends AbstractMoobloom implements GeoEntity {
    public MelonMoobloom(EntityType<? extends AbstractMoobloom> type, Level level) {
        super(type, level);
    }
    ...
    public void tick() {
        ...
        if (tickCounter >= 600) {
            ...
            for (int x = -5; x <= 5; x++) {
                for (int y = -5; y <= 5; y++) {
                    for (int z = -5; z <= 5; z++) {
                        BlockPos blockpos = MelonMoobloom.this.blockPosition().offset(x, y, z);
                        BlockState blockstate = MelonMoobloom.this.level().getBlockState(blockpos);
                        Block block = blockstate.getBlock();

                        if (blockstate.is(Blocks.MELON_STEM)) {
                            if (block instanceof StemBlock) {
                                ...
                                if (!blockstate.equals(newState)) {
                                    MelonMoobloom.this.level().setBlockAndUpdate(blockpos, newState);
                                }
    ...
    public static final EntityDataAccessor<Integer> OVERLAY = SynchedEntityData.defineId(MelonMoobloom.class, EntityDataSerializers.INT);
    public static final EntityDataAccessor<Integer> HORN_TYPE = SynchedEntityData.defineId(MelonMoobloom.class, EntityDataSerializers.INT);
    protected static final EntityDataAccessor<Integer> BRAND_TAG_COLOR = SynchedEntityData.defineId(MelonMoobloom.class, EntityDataSerializers.INT);
    public static final EntityDataAccessor<Boolean> TAGGED = SynchedEntityData.defineId(MelonMoobloom.class, EntityDataSerializers.BOOLEAN);
    public static final EntityDataAccessor<Boolean> MILKED = SynchedEntityData.defineId(MelonMoobloom.class, EntityDataSerializers.BOOLEAN);
    public static final EntityDataAccessor<Boolean> HARNESSED = SynchedEntityData.defineId(MelonMoobloom.class, EntityDataSerializers.BOOLEAN);
    public static final EntityDataAccessor<Boolean> BELLED = SynchedEntityData.defineId(MelonMoobloom.class, EntityDataSerializers.BOOLEAN);

    public static final EntityDataAccessor<Integer> VARIANT = SynchedEntityData.defineId(MelonMoobloom.class, EntityDataSerializers.INT);
    public ResourceLocation getTextureLocation() {
        return MelonMoobloomModel.Variant.variantFromOrdinal(getVariant()).resourceLocation;
    }
    ...
    public static final EntityDataAccessor<Boolean> SHEARED = SynchedEntityData.defineId(MelonMoobloom.class, EntityDataSerializers.BOOLEAN);
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor serverLevelAccessor, DifficultyInstance instance, MobSpawnType spawnType, @Nullable SpawnGroupData data) {
        ...
        setVariant(random.nextInt(MelonMoobloomModel.Variant.values().length));
