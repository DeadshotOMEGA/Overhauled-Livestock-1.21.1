...
public class WheatMoobloom extends AbstractMoobloom implements GeoEntity {
    public WheatMoobloom(EntityType<? extends AbstractMoobloom> type, Level level) {
        super(type, level);
    }
    ...
    public void tick() {
        ...
        if (tickCounter >= 600) {
            ...
            for (int x = -5; x <= 5; x++) {
                for (int y = -5; y <= 5; y++) {
                    for (int z = -5; z <= 5; z++) {
                        BlockPos blockpos = WheatMoobloom.this.blockPosition().offset(x, y, z);
                        BlockState blockstate = WheatMoobloom.this.level().getBlockState(blockpos);
                        ...
                        if (blockstate.is(Blocks.WHEAT)) {
                            if (block instanceof CropBlock) {
                                ...
                                if (!blockstate.equals(newState)) {
                                    WheatMoobloom.this.level().setBlockAndUpdate(blockpos, newState);
                                }
    ...
    public static final EntityDataAccessor<Boolean> BELLED = SynchedEntityData.defineId(WheatMoobloom.class, EntityDataSerializers.BOOLEAN);

    public static final EntityDataAccessor<Integer> VARIANT = SynchedEntityData.defineId(WheatMoobloom.class, EntityDataSerializers.INT);
    public ResourceLocation getTextureLocation() {
        return WheatMoobloomModel.Variant.variantFromOrdinal(getVariant()).resourceLocation;
    }
    ...
    public static final EntityDataAccessor<Boolean> SHEARED = SynchedEntityData.defineId(WheatMoobloom.class, EntityDataSerializers.BOOLEAN);
