...
@OnlyIn(Dist.CLIENT)
public class OLlamaChestLayer extends GeoRenderLayer<OLlama> {
    public OLlamaChestLayer(GeoRenderer<OLlama> entityRendererIn) {
    ...
    public void render(PoseStack poseStack, OLlama animatable, BakedGeoModel bakedModel, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
        ...
        if (animatable.hasChest()) {
            resourceLocation = ResourceLocation.fromNamespaceAndPath(LivestockOverhaul.MODID, "textures/entity/llama/tack/saddlebags.png");
        } else {
            return;
        }
