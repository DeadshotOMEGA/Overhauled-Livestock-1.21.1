...
public class CaribouCarpetLayer extends GeoRenderLayer<Caribou> {
    ...
    public CaribouCarpetLayer(GeoRenderer<Caribou> entityRendererIn) {
    ...
    public void render(PoseStack poseStack, Caribou animatable, BakedGeoModel bakedModel, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
        ...
        if (!armorItemStack.isEmpty() && !itemStack.isEmpty() && !(itemStack.getItem() instanceof CaparisonItem) && !(itemStack.getItem() instanceof CosmeticsItem)) {
